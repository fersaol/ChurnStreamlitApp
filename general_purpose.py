...
def module_index():
    index = pd.DataFrame({
        "function_name":["varios_plot",
                            "escalado"],
        "purpose":["to plot in a single figure multiple histograms",
                    "to scale data with your chosen scaler"]
    })
    return index
...
def cross_validation_report(estimador:any,x:pd.DataFrame,y:pd.DataFrame,
    ...
    --------------------------------------
    # Args:
        - estimador: algoritmo de clasificación de sklearn que usaremos con el cross_val_score\n
        - x: (pd.DataFrame) features de x_train\n
        - y: (pd.DataFrame) target o y_train \n
...
def Binary_classification_assessment(precisions,
                                        recalls,
                                        thresolds,
                                        scores,
                                        y_train,
                                        width=15,
                                        height=5,
                                        metric_selection="auto"):

    if metric_selection == "auto":
        metric_value = thresolds[np.argmax(precisions==recalls)]
    else:
        metric_value = metric_selection
        
    y_pred_threshold = (scores > metric_value)
    fig = plt.figure(figsize=(width,height))
    fig.subplots_adjust(hspace=0.8)

    fig.add_subplot(2,2,1)
    # Precission vs Recall Plot
    plt.plot(recalls, precisions, 'b-', linewidth=2, label="Precission VS Recall")
    plt.xlabel('Recall-Thresold', fontsize=16)
    plt.xlim(0)
    ...
    # Precission-Recall Trade-Off Curve Plot
    plt.title("Precision Recall Trade Off")
    plt.plot(thresolds, precisions[:-1], "y--", label="Precision")
    plt.plot(thresolds, recalls[:-1], "g-", label="Recall")
    plt.xlim(0)
    plt.legend(loc="upper right")

    fig.add_subplot(2,2,3)
    new_classification_report(y_train,y_pred_threshold)


    plt.show()

def my_rocAuc(y_train,scores):
    fpr,tpr,thresolds = roc_curve(y_train,scores)
    auc = roc_auc_score(y_train,scores,average="weighted")
    
    plt.fill_between(fpr,tpr)
    plt.plot([0,1],[0,1],"orange")
    plt.annotate("AUC:" + str(round(auc,3)),xy=[0.8,0.2])
    plt.xlabel("FPR (1-specificity(TNR true negative rate))")
